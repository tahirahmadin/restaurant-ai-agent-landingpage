...
const HouseComponent: React.FC<HouseComponentProps> = ({ className = "", imageSrc, houseName }) => {
  const { orders, weather, showCoinAnimation, completeCoinAnimation } = useStore();
  ...
  const getStatusText = () => {
    ...
    switch (houseOrder.status) {
      ...
      case 'out_for_delivery':
        const timeLeft = calculateTimeLeft(houseOrder.deliveryProgress || 0, DELIVERY_PATHS[houseName], weather);
  ...
  return (
    <div className={`relative w-20 h-20 flex flex-col items-center justify-center transform hover:scale-105 transition-transform ${className}`}>
      {statusText && (
...
export const GridMap: React.FC = () => {
  ...
  return (
    <div 
      ...
      style={{
        backgroundImage: 'url("https://gobbl-bucket.s3.ap-south-1.amazonaws.com/tapAssets/gobblai/map.png")',
    ...
    >
      ...
      <div className="absolute left-20 right-20 top-1/2 -translate-y-1/2 h-8 bg-[#1B1B1B] shadow-lg z-5">
        ...
        <div className="absolute inset-0 opacity-10 mix-blend-overlay"
             style={{
               backgroundImage: 'url("https://images.unsplash.com/photo-1585732832634-77c8a8683f67?q=80&w=500&auto=format")',
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="h-[3px] w-full"
               style={{
                 backgroundImage: 'linear-gradient(90deg, rgba(255, 255, 255, 0.8) 50%, transparent 50%)',
                 backgroundSize: '24px 100%',
                 boxShadow: weather === 'sunny' ? '0 0 5px rgba(255, 255, 255, 0.15)' : '0 0 10px rgba(255, 255, 255, 0.3)'
               }} />
        </div>
      </div>
      ...
      <div className="absolute top-20 bottom-20 left-1/2 -translate-x-1/2 w-8 bg-[#1B1B1B] shadow-lg z-5">
        ...
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="w-[3px] h-full"
               style={{
                 backgroundImage: 'linear-gradient(0deg, rgba(255, 255, 255, 0.8) 50%, transparent 50%)',
      ...
      <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex -space-x-2 z-20">
        {activeDeliveries.length === 0 && (
          <DeliveryBike progress={0} paths={DELIVERY_PATHS['Habib House']} orderId="default" />
        )}
      </div>
      ...
      {activeDeliveries.slice(0, 1).map((order) => (
        <DeliveryBike
          ...
          paths={DELIVERY_PATHS[order.destination] || DELIVERY_PATHS['Habib House']}
