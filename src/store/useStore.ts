...
export const useStore = create<StoreState>((set, get) => ({
  ...
  addOrder: (order) =>
    set((state) => ({
      orders: [
        ...state.orders,
        { ...order, deliveryProgress: 0, isReturning: false },
      ],
  ...
  updateOrderStatus: (orderId, status) => {
    ...
    if (status === "delivered" && order) {
      ...
      set((state) => ({
        ...
        totalRevenue:
          status === "delivered"
            ? state.totalRevenue + orderAmount
            : state.totalRevenue,
      }));
    }
  },
  ...
  startReturnJourney: (orderId: string) =>
    set((state) => ({
      orders: state.orders.map((order) => {
        if (
          order.id === orderId &&
  ...
  simulateOrder: () => {
    ...
    const hasActiveDelivery = state.orders.some(
      (order) =>
        order.status === "out_for_delivery" ||
        (order.status === "delivered" && order.isReturning)
    );
    const hasPreparingOrder = state.orders.some(
      ...
      if (!currentOrder || currentOrder.status === "completed") {
        ...
        if (currentOrder?.status === "completed") {
          ...
          setTimeout(() => {
            ...
            const nextOrder = get().orders.find(
              (o) =>
